---
title: "How to get printk format specifiers right — The Linux Kernel  documentation"
source: "https://docs.kernel.org/core-api/printk-formats.html"
author:
  - "[[default ESCAPE_ANY_NP is used.]]"
published:
created: 2026-06-24
description:
tags:
...
Randy Dunlap < [rdunlap@infradead.org](mailto:rdunlap%40infradead.org) >

Author:

Andrew Murray < [amurray@mpc-data.co.uk](mailto:amurray%40mpc-data.co.uk) >
...
If \<type> is architecture-dependent for its size (e.g., cycles\_t, tcflag\_t) or is dependent on a config option for its size (e.g., blk\_status\_t), use a format specifier of its largest possible type and explicitly cast to it.
...
The kernel’s printf does not support %n. Floating point formats (%e, %f, %g, %a) are also not recognized, for obvious reasons. Use of any unsupported specifier or length qualifier results in a WARN and early return from [`vsnprintf()`](https://docs.kernel.org/core-api/kernel-api.html#c.vsnprintf "vsnprintf").
...
If (and only if) you are printing addresses as a content of a virtual file in e.g. procfs or sysfs (using e.g. `seq_printf()`, not [`printk()`](https://docs.kernel.org/core-api/printk-basics.html#c.printk "printk")) read by a userspace process, use the %pK modifier described below instead of %p or %px.
...
For printing error pointers (i.e. a pointer for which [`IS_ERR()`](https://docs.kernel.org/core-api/kernel-api.html#c.IS_ERR "IS_ERR") is true) as a symbolic error name. Error values for which no symbolic name is known are printed in decimal, while a non-ERR\_PTR passed as the argument to %pe gets treated as ordinary %p.
...
The `k` and `u` specifiers are used for printing prior probed memory from either kernel memory (k) or user memory (u). The subsequent `s` specifier results in printing a string. For direct use in regular [`vsnprintf()`](https://docs.kernel.org/core-api/kernel-api.html#c.vsnprintf "vsnprintf") the (k) and (u) annotation is ignored, however, when used out of BPF’s `bpf_trace_printk()`, for example, it reads the memory it is pointing to without faulting.
...
For printing kernel pointers which should be hidden from unprivileged users. The behaviour of %pK depends on the kptr\_restrict sysctl - see [Documentation for /proc/sys/kernel/](https://docs.kernel.org/admin-guide/sysctl/kernel.html) for more details.

This modifier is *only* intended when producing content of a file read by userspace from e.g. procfs or sysfs, not for dmesg. Please refer to the section about %p above for discussion about how to manage hashing pointers in [`printk()`](https://docs.kernel.org/core-api/printk-basics.html#c.printk "printk").
...
Before using %px, consider if using %p is sufficient together with enabling the `no_hash_pointers` kernel parameter during debugging sessions (see the %p description above). One valid scenario for %px might be printing information immediately before a panic, which prevents any sensitive information to be exploited anyway, and with %px there would be no need to reproduce the panic with no\_hash\_pointers.
...
For printing the pointer differences, use the %t modifier for ptrdiff\_t.
...
For printing `struct resources`. The `R` and `r` specifiers result in a printed resource with (R) or without (r) a decoded flags member. If start is equal to end only print the start value.

Passed by reference.

### Physical address types phys\_addr\_t

```
%pa[p]  0x01234567 or 0x0123456789abcdef
```

For printing a phys\_addr\_t type (and its derivatives, such as resource\_size\_t) which can vary based on build options, regardless of the width of the CPU data path.
...
For printing `struct range`. `struct range` holds an arbitrary range of u64 values. If start is equal to end only print the start value.

Passed by reference.

### DMA address types dma\_addr\_t

```
%pad    0x01234567 or 0x0123456789abcdef
```

For printing a dma\_addr\_t type which can vary based on build options, regardless of the width of the CPU data path.
...
The conversion rules are applied according to an optional combination of flags (see [`string_escape_mem()`](https://docs.kernel.org/core-api/kernel-api.html#c.string_escape_mem "string_escape_mem") kernel documentation for the details):
...
```ad-summary
小型的buffer(不超过64 byte) 适合使用printk 使用上面的format打印，但是大的buffer最好使用 `print_hex_dump()`
```
...
The additional `c` specifier can be used with the `I` specifier to print a compressed IPv6 address as described by [https://tools.ietf.org/html/rfc5952](https://tools.ietf.org/html/rfc5952)
...
For printing an IP address without the need to distinguish whether it’s of type AF\_INET or AF\_INET6. A pointer to a valid `struct sockaddr`, specified through `IS` or `iS`, can be passed to this format specifier.
...
In case of an IPv6 address the compressed IPv6 address as described by [https://tools.ietf.org/html/rfc5952](https://tools.ietf.org/html/rfc5952) is being used if the additional specifier `c` is given. The IPv6 address is surrounded by `[`, `]` in case of additional specifiers `p`, `f` or `s` as suggested by [https://tools.ietf.org/html/draft-ietf-6man-text-addr-representation-07](https://tools.ietf.org/html/draft-ietf-6man-text-addr-representation-07)
...
For printing dentry name; if we race with `d_move()`, the name might be a mix of old and new ones, but it won’t oops. %pd dentry is a safer equivalent of %s dentry->d\_name.name we used to use, %pd\<n> prints `n` last components. %pD does the same thing for [`struct file`](https://docs.kernel.org/filesystems/api-summary.html#c.file "file").
...
For printing `struct va_format` structures. These contain a format string and va\_list as follows:
...
For printing information on an fwnode\_handle. The default is to print the full node name, including the path. The modifiers are functionally equivalent to %pOF above.
...
The %ptSp is equivalent to %lld.%09ld for the content of the `struct timespec64`. When the other specifiers are given, it becomes the respective equivalent of %ptT\[dt\]\[r\]\[s\].%09ld. In other words, the seconds are being printed in the human readable format followed by a dot and nanoseconds.
...
For printing `struct clk` structures. %pC prints the name of the clock (Common Clock Framework) or a unique 32-bit ID (legacy clock framework).
...
### Flags bitfields such as page flags and gfp\_flags
...
> - v - \[v\]ma\_flags, expects value of type (`unsigned long *`)
> - g - \[g\]fp\_flags, expects value of type (`gfp_t *`)

The flag names and print order depends on the particular type.

Note that this format should not be used directly in the `TP_printk()` part of a tracepoint. Instead, use the show\_\* `_flags()` functions from <trace/events/mmflags.h>.
...
Print a generic FourCC code, as both ASCII characters and its numerical value as hexadecimal.
...
The additional `h`, `hR`, `l`, and `b` specifiers define what endianness is used to load the stored bytes. The data might be interpreted using the host, reversed host byte order, little-endian, or big-endian.
...
If you add other %p extensions, please extend <lib/tests/printf\_kunit.c> with one or more test cases, if at all feasible.
