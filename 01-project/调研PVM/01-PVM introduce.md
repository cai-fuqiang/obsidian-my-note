...
![PVM引入背景](01-project/调研PVM/文献笔记/PVM-引入背景.md)
...
[[01-project/调研PVM/excalidraw/VMX 扩展.excalidraw#^fkGaA3v2|VMX CPU]]

![[01-project/调研PVM/excalidraw/VMX 扩展.excalidraw|600]]
...
![[01-project/调研PVM/excalidraw/mem.excalidraw|1000]]
...
![[01-project/调研PVM/文献笔记/nested virt中内存性能下降严重]]
...
![[01-project/调研PVM/excalidraw/nested VMCS.excalidraw|600]]
...
![[01-project/调研PVM/excalidraw/VM-exit forward.excalidraw|600]]
...
![[01-project/调研PVM/文献笔记/嵌套虚拟化中地址转换]]
...
| -------------------------------------------- | ---------------------------------------------------- | ------------------------------- |
| [[01-project/调研PVM/文献笔记/nested mem virt - SPT-on-EPT\|SPT-on-EPT]] | GVA<sub>L2</sub>->GPA<sub>L2</sub>->GPA<sub>L1</sub> | L1当EPT feature 不存在，在L1中使用影子页表   |
| [[01-project/调研PVM/文献笔记/nested mem virt - EPT-on-EPT\|EPT-on-EPT]] | GPA<sub>L2</sub>->GPA<sub>L1</sub>->HPA<sub>L0</sub> | L0中使用影子页表，只不过影子EPT<sub>12</sub> |
![[01-project/调研PVM/文献笔记/内存嵌套虚拟化两种实现性能均不佳]]
...
#### SPT-on-EPT
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=5&rect=39,538,290,732|sosp2023-pvm-paper, p.5]]
主要分两个阶段:
1. [[01-project/调研PVM/文献笔记/SPT-on-EPT建立GPT阶段触发PF#PF处理流程|L2建立页表阶段]]
2. L2建立完页表, 但是leaf SPT中并未填入具体物理页而导致`#PF`, 这时L1会补全GPT<sub>L1</sub> 以及 SPT, 其在[[01-project/调研PVM/文献笔记/SPT-on-EPT二阶段和一阶段不同的点|这些步骤有些不同(4,7)]]。

![[01-project/调研PVM/文献笔记/SPT-on-EPT最坏情况下world switch总次数]]

> [!tip]
> [[SPT readonly是导致一阶段world switch 主要因素]]
#### EPT-on-EPT
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=5&rect=314,538,571,737|sosp2023-pvm-paper, p.5]]

> [!PDF|red] [[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=5&selection=483,0,485,5&color=red|📖 |EPT-on-EPT是当前最优秀的 nested memory virtualization 实现]]

其工作也分两个阶段:
* [[01-project/调研PVM/文献笔记/EPT-on-EPT第一阶段| 构建 EPT12]]
* [[01-project/调研PVM/文献笔记/EPT-on-EPT第二阶段工作|根据 EPT01 && EPT12 构建 EPT02]]

EPT-on-EPT优点和不足:
* [[01-project/调研PVM/文献笔记/EPT-on-EPT优点| 优点:  L2 更新 GPT2不会再发生 world switch]]
* [[01-project/调研PVM/文献笔记/EPT-on-EPT不足|不足: build 和 update EPT02仍然需要 world switch]]

![[01-project/调研PVM/文献笔记/EPT-on-EPT world switch总次数]]
...
![[01-project/调研PVM/文献笔记/EPT-on-EPT的缺点总结]]
### interrupt virtualization with nested

(暂不关注 #TODO )
## summary of nested virt
* [[01-project/调研PVM/文献笔记/嵌套虚拟化中硬件加速不明显，甚至可能起到反作用]]
* [[01-project/调研PVM/文献笔记/目前嵌套虚拟化的设计需求并不贴合以IAAS为底座的安全容器的场景]] ==**(关键)**==

<font color="#ff0000"><mark style="background:#b1ffff">So, create PVM to solve it!!</mark></font>
# PVM
## overview
* 什么是PVM?
  ![[01-project/调研PVM/文献笔记/PVM概述]]
* PVM 目标?
  ![[01-project/调研PVM/文献笔记/PVM目标- 轻量, 简单，高效]]
* PVM组成
  ![[01-project/调研PVM/文献笔记/PVM三大组件]]
  [[01-project/调研PVM/文献笔记/PVM 涉及的两个Hypervisor 内核模块|hypervisor涉及两个内核模块: kvm.ko, kvm-pvm.ko]]
* 设计简述
  ![[01-project/调研PVM/文献笔记/PVM 设计简述]] 
## PVM implement
### PVM CPU virt  - base vring

![[01-project/调研PVM/文献笔记/PVM中L1 hypervisor, L2 用户态程序, L2 内核在ring中的位置]]
...
![[01-project/调研PVM/文献笔记/PVM switcher 映射位置]]
并且, [[01-project/调研PVM/文献笔记/switcher 需要在多个地址空间中映射相同地址原因|switcher 需要在多个地址空间中映射相同地址]]

另外, ![[01-project/调研PVM/文献笔记/switcher 仿照 xen & lguest 等现有半虚拟化 架构设计，并做了改进]]
> [!note] 软件虚拟化的隔离肯定没有硬件好。所以PVM在这个方面做了很多努力(安全容器的一个重要诉求，就是提升不同容器的隔离性)
> 


![[01-project/调研PVM/文献笔记/switcher 组成包括高效简洁的汇编代码和数据结构]]
关键结构:
...
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=7&rect=313,554,555,722|sosp2023-pvm-paper, p.7]]
...
第二种方式, ==direct switch==:
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=8&rect=59,575,290,735|sosp2023-pvm-paper, p.8]]
...
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=8&rect=321,560,542,734|sosp2023-pvm-paper, p.8]]
* 更便宜的world switch
...
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=10&rect=309,615,565,736|sosp2023-pvm-paper, p.10]]
* 在single-level 中, PVM VM-entry VM-exit的处理时间 仅比 kvm 略慢
* 而对比`KVM(NST)`, `pvm (NST)`, 其速度要快接近一个数量级

### syscall
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=11&rect=38,602,302,738|sosp2023-pvm-paper, p.11]]
...
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=11&rect=302,601,568,751|sosp2023-pvm-paper, p.11]]
...
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=12&rect=43,464,302,724|sosp2023-pvm-paper, p.12]]
...
***
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=13&rect=56,582,576,720|sosp2023-pvm-paper, p.13]]
...
![[01-project/调研PVM/pdf/sosp2023-pvm-paper.pdf#page=13&rect=50,366,302,579|sosp2023-pvm-paper, p.13]]
...
![[01-project/调研PVM/文献笔记/PVM 性能评估总结- real world app]]
...
# 附录
* [[01-project/调研PVM/文献笔记/EPT-on-EPT 比 SPT-on-EPT 触发PF的总次数更少]]
* [[01-project/调研PVM/文献笔记/EPT-on-EPT 比 SPT-on-EPT 在每次PF时world switch 切换更少]]
