---
title: PVM get started with kata
source: https://github.com/virt-pvm/misc/blob/main/pvm-get-started-with-kata.md
author:
published:
created: 2026-08-04
description: Miscellaneous resource about PVM. Contribute to virt-pvm/misc development by creating an account on GitHub.
tags:
  - clippings
share_link: https://share.note.sx/n3nncxcy#48VwJYgpeJEUvvw8z4HHlg
share_updated: 2026-08-05T15:07:13+08:00
---
...
`PVM` is a software virtualization technology that is <mark style="background:#d3f8b6">purpose-built</mark> to support **`Kata Containers`** **without the need for ==hardware virtualization== assistance**. It is designed as a vendor for `KVM`, similar to `Intel` and `AMD`, making it compatible with the software stack in `Kata Containers`.
...
```
- **Verify**  
	You are now ready to run Kata Containers with KVM. First, ensure the `KVM` and `VSOCK` modules are loaded.
```
...
In addition, there are some issues when using QEMU with PVM. If you wish to test QEMU with `snapshot` and `migration`, we recommend either picking the [commits](https://github.com/qemu/qemu/compare/master...virt-pvm:qemu:pvm_qemu_migration) from our QEMU repository. ^691d34

## Configure Cloud Hypervisor for PVM

Due to the [virtio-vsock issue](https://github.com/cloud-hypervisor/cloud-hypervisor/issues/5691), the rust vmm (including Dragonball, Cloud Hypervisor and Firecracker) installed in the Kata Container package cannot support PVM guest. We have only found that Cloud Hypervisor has fixed the issue, so we should use version 35.0 or higher of Cloud Hypervisor. ^d17536
...
**Note：Due to this [issue](https://github.com/virt-pvm/linux/issues/1), the guest fails to boot successfully when using Cloud Hypervisor with a virtio-pmem rootfs under PVM. However, the workaround in the issue cannot be utilized by Kata Containers at the moment. Therefore, if you wish to test Cloud Hypervisor for PVM, you must use the modified runtime binary provided in our [package](https://github.com/virt-pvm/misc/releases/download/test/pvm-kata-vm-img.tar.gz). And add the following option in configuration file.** ^7953de
